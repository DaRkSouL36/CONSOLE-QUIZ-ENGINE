...
private:
    ...
    int lifelinesRemaining;
    
    // PURPOSE: PROMPTS THE USER FOR AN ANSWER, ALLOWING FOR A HINT IF APPLICABLE
    // INPUT: BOOLEAN INDICATING IF THE HINT HAS ALREADY BEEN USED ON THIS SPECIFIC QUESTION
    // OUTPUT: A VALIDATED UPPERCASE CHARACTER (A, B, C, D, H, OR X)
    // LOGIC: DYNAMICALLY GENERATES PROMPT TEXT BASED ON LIFELINE STATE
    char getUserInput(bool hintUsedThisQuestion); 
